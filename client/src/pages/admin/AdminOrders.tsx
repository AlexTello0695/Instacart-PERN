...
export default function AdminOrders() {
    ...
    const statusOptions = ["Realizado", "Confirmado", "Asignado", "Empacado", "En Ruta", "Entregado", "Cancelado"];
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-app-cream/50 text-zinc-500 uppercase text-xs font-semibold">
                            <tr>
                                <th className="px-6 py-4">Detalles del Pedido</th>
                                <th className="px-6 py-4">Cliente</th>
                                <th className="px-6 py-4">Total</th>
                                <th className="px-6 py-4">Socio Repartidor</th>
                        ...
                        <tbody className="divide-y divide-app-border">
                            {orders.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-zinc-500">No se encontraron pedidos.</td>
                                </tr>
                            ) : (
                                        ...
                                        <td className="px-6 py-4">
                                            {order.deliveryPartner ? (
                                                ...
                                                <button onClick={() => { setAssignModal(order._id); setSelectedPartner(""); }} className="px-3 py-1.5 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 transition-colors flex items-center gap-1">
                                                    <TruckIcon className="size-3" /> Asignar
                                                </button>
                                            )}
                                        </td>
                                        <td className="px-6 py-4">
                ...
                <>
                    ...
                    <div className="fixed inset-0 z-50 flex-center p-4">
                        <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
                            <h3 className="text-lg font-semibold text-app-green mb-4">Asignar Socio Repartidor</h3>
                            {partners.length === 0 ? (
                                <p className="text-sm text-zinc-500 mb-4">Sin Socios Repartidores Activos. Por favor, agrega un socio repartidor.</p>
                            ) : (
