...
const CartSidebar = () => {
  ...
  return (
    <>
        ...
        {/* {Sidebar} */}
        <div className="fixed bg-white right-0 top-0 h-full w-full max-w-md z-50 shadow-2xl flex flex-col animate-slide-in-right">
          {/* { Header } */}
          <div className="flex items-center justify-between p-5 border-b border-app-border">
            <div className="flex items-center gap-2">
              <ShoppingBagIcon className="size-5"/>
              <h2 className="text-lg font-medium">Tu carrito</h2>
              <span className="px-2 py-0.5 text-xs font-semibold bg-app-cream rounded-full">{items.length} objetos</span>
            </div>
            <button 
            onClick={()=> setIsCartOpen(false)}
          ...
          {items.length > 0 && (
            <div className="p-5 border-t border-app-border space-y-3">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-app-text-light">Envío</span>
                <span className="font-medium">{deliveryFee === 0 ? <span className="text-app-success">Gratis</span> : `${currency}${deliveryFee.toFixed(2)}`}</span>
              </div>

              {deliveryFee > 0 && <p className="text-xs text-app-text-light text-center">¡Envío gratuito en pedidos mayores a {currency}20!</p>}
              ...
              <button
                ...
                className="w-full py-3 bg-app-orange text-white font-semibold rounded-xl hover:bg-app-orange-dark transition-colors flex-center gap-2 active:scale-[0.98]">
                  Proceder al pago <ArrowRightIcon className="size-4" />
              </button>
            </div>
          )}
        </div>
