import { ArrowUpRightIcon, BadgePercentIcon, BikeIcon, ChevronDownIcon, LogOutIcon, MapPinIcon, MenuIcon, PackageIcon, SearchIcon, ShieldIcon, ShoppingCartIcon, UserIcon, XIcon } from "lucide-react";
import React, { useState } from "react";
import { Link, useNavigate } from "react-router-dom";

const NavBar = () => {

  const user: any = {name: "Alex", email: "alext@ejemplo.com", isAdmin: "true"}
            ...
            <div className="relative">
              ...
              : (
                <div className="flex-center gap-2">
                  <Link to='/login' className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-950 rounded-full hover:bg-green-950-ligh transition-colors ">
                ...
                <>
                  <div className="fixed inset-0 z-40" onClick={()=> setUserMenuOpen(false)} />
                    <div className="absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in">
                      ...
                      {user && <Link to='/orders' className="dropdown-link"><PackageIcon size={16} /> Mis Pedidos </Link>}

                      {user && <Link to='/addresses' className="dropdown-link"><MapPinIcon size={16} /> Mis Direcciones </Link>}

                      <Link to='/products' className="dropdown-link md:hidden"><ArrowUpRightIcon size={16} /> Productos </Link>

                      <Link to='/deals' className="dropdown-link md:hidden"><BadgePercentIcon size={16} /> Ofertas </Link>
