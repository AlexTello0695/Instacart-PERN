    ...
    return (
        <div className="bg-white rounded-2xl p-6 animate-fade-in">
            <h2 className="text-lg font-semibold text-app-green mb-5 flex items-center gap-2">
                <MapPinIcon className="size-5" /> Dirección de Entrega
            </h2>
            {user?.addresses && user.addresses.length > 0 && (
                <div className="mb-6">
                    <h3 className="text-sm font-semibold text-app-green mb-3">Direcciones Guardadas</h3>
                        ...
                        {user.addresses.map((addr: any) => (
                            ...
                            >
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    {addr.isDefault && <span className="text-[10px] font-semibold text-app-orange uppercase tracking-wider bg-orange-50 px-2 py-0.5 rounded-full">Predeterminado</span>}
                                </div>
            ...
            <Link to="/addresses" className="mt-6 px-6 py-3 border border-gray-600 text-gray-600 rounded-xl flex-center gap-2">
                Agregar Nueva Dirección <PlusIcon className="size-4" />
            </Link>
